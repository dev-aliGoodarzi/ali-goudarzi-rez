...
const Setting = ({ setThemeColor, themeColor, setIsRTL }) => {
    ...
    return <div className={`${styles.settingContainer} ${isSettingOpen && styles.open}`}>
        <FiSettings
            onMouseEnter={(event) => {
        ...
        <div className={styles.contnets}>
            ...
            <div className={styles.colorSwitcherContainer}>
                {
                    colors.map(item => <div key={item.id} className={styles.colorSwitcherItem} onClick={() => setThemeColor(item.colorCode)} style={{ background: item.colorCode }}></div>)
                }
            </div>
            <div className={styles.RTLCheacker}>
                <button onClick={() => setIsRTL(true)}>RTL</button>
                <button onClick={() => setIsRTL(false)}>LTR</button>
            </div>
        </div>
    </div>
